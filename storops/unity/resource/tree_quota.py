...
class UnityTreeQuota(UnityResource):
    ...
    @classmethod
    def create(cls, cli, file_system_id=None, path=None,
               description=None, hard_limit=None,
               soft_limit=None):
        """
        Creates tree_quota on the specified filesystem and path
        :param file_system_id: This parameter will help create
            tree_quota on the file_system and folder path specified
        ...
        tree_quota_param = cls.prepare_tree_quota_create_parameters(
            file_system, path, description, hard_limit, soft_limit)
        resp = cli.post(
    ...
    def delete(cls, cli, tree_quota_id):
        ...
        if not tree_quota.existed:
            raise UnityResourceNotFoundError(
                'cannot find tree_quota {}.'.format(cls.get_id()))
    ...
    def prepare_tree_quota_create_parameters(
        ...
        :param file_system: Filesystem on which tree_quota needs to
            be created
        :param path: Path of the tree quota relative to the root of
            the file system
